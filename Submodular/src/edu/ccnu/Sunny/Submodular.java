...
public class Submodular {
    ...
    public void Summarize(String args[]) throws IOException
    {
        ...
        //multi document
            ...
            int sx = Integer.parseInt(args[5]);//stemmerOrNot
            ...
            BufferedWriter bf = new BufferedWriter(new PrintWriter(outfile,"utf-8"));
...
//                bf.append("\n");
    ...
    public double submod1(int id)
    {
        ...
        for (int i = 0; i < myDoc.snum; i++){
            if (i == id) continue;
            double sum = 0;
            //计算在summary里的句子i的所有的余弦相似度和sum
            for (int j : myDoc.summary_id)
            if (i != j)
                sum += myDoc.sim[i][j];
            //id!=-1，sum+句子i和句子id的余弦相似度
            if (id != -1)
                sum += myDoc.sim[i][id];
    ...
    public double submod2(int id){
        double score=0;
        //找出最大的和句子id相似的句子i的余弦相似度sim[id][i]
        for (int i : myDoc.summary_id) {           
    ...
    public void greedy(){
        boolean[] chosen = new boolean[myDoc.snum];//标记句子是否被选入
        int len=0;
        calc_sumSim();
        while (true){
            double
            maxInc = -10, initScore = submod1(-1);
            int maxId = -1;
            for (int i = 0; i < myDoc.snum; i++){
                if (!chosen[i] && len+myDoc.sen_len.get(i)<myDoc.maxlen){
                    double inc = (Lambda * submod1(i) +(1-Lambda) * submod2(i) - initScore*Lambda)
                    		/Math.pow(myDoc.sen_len.get(i),Beta);
                    if (inc > maxInc){
                        maxInc = inc;
                        maxId = i;
                    }
